...
def safe_write_csv(df: pd.DataFrame, out_path: Path, encoding: str = ENCODING):
    try:
        ...
        try:
            ...
            print(f"   ⚠️ Sanitized name. Saved → {spath.name}")
            return
        except Exception:
            out_path = spath
    ...
    while True:
        ...
        try:
            ...
            print(f"   ⚠️ Original file busy. Saved → {candidate.name}")
            return
...
def map_new_to_old_schema(new_df: pd.DataFrame, old_cols_ordered: List[str]) -> pd.DataFrame:
    ...
    old_addr_col = None
    for col in old_cols_ordered:
        if re.sub(r"[\s_]+", "", col.lower()) in {re.sub(r"[\s_]+", "", c.lower()) for c in ADDR_CANDS}:
            old_addr_col = col
            break
...
def compare_pair(old_df: pd.DataFrame, new_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return out[["flag", "changed_columns"] + schema]
...
def compare_uploaded_files(old_file, new_file) -> pd.DataFrame:
    ...
    Returns the compared DataFrame.
    """
    ...
    old_df = normalize_headers(old_df)
    new_df = normalize_headers(new_df)

    result = compare_pair(old_df, new_df)
    return result

# Note: module intended to be imported. No auto-run on import.
